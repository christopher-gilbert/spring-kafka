 ...
 * it. Any bean annotated as a {@link KafkaKeyDeserializer} will be a prototype scoped bean with each consumer
 ...
 * the key deserializer class in application properties.
 * <p>
 ...
 * <p>
 * For example usage, the following would apply the customDeserializer to keys of records consumed by the
 * customContainerFactory's consumers, and the anotherCustomDeserializer to keys of records consumed by
 ...
 * <p>
 * In both examples, the value deserializer has not been specified - either a valueDeserializer class should be
 * specified in application properties, else another Deserializer should be annotated with {@link KafkaValueDeserializer}
 *
 * <pre class="code">
 *
 * &#064;Bean
 ...
 *
 * &#064;Bean
 * &#064;KafkaKeyDeserializer(consumerFactories = "customConsumerFactory")
 ...
 *
 * &#064;Bean
 * public KafkaListenerContainerFactory&lt;ConcurrentMessageListenerContainer&lt;AnotherCustomKey, CustomEvent&gt;&gt;anotherCustomContainerFactory() {
 *     KafkaConsumerFactory&lt;AnotherCustomKey, CustomEvent&gt; consumerFactory = customConsumerFactory();
 *     KafkaListenerContainerFactory factory = new ConcurrentKafkaListenerContainerFactory&lt;AnotherCustomKey, CustomEvent&gt;();
 *     factory.consumerFactory = new KafkaConsumerFactoryWithDeserializerFactory&lt;AnotherCustomKey, CustomEvent&gt;(kafkaProperties.buildConsumerProperties());
 *     factory.setConcurrency(5);
 *     return factory;
 * }
 * </pre>
...
public @interface KafkaKeyDeserializer {

	/**
	 * Bean names for specific ConsumerFactories for which the annotated Deserializer is applicable. If omitted then
	 * the Deserializer will apply to any ConsumerFactories that do not have a specific key deserializer.
	 *
